...
class PointsFeatureTests(TestCase):
    ...
    def test_points_are_per_user(self):
        ...
        self.client.post("/api/stores/", {"name": "My Store", "address": "Tokyo"})
        other_client.post("/api/stores/", {"name": "Other Store", "address": "Osaka"})

        response = self.client.get("/api/user/points/")
        self.assertEqual(response.data["total_points"], 3)

        response = other_client.get("/api/user/points/")
        self.assertEqual(response.data["total_points"], 3)
